...
export function Navbar({ session }: NavbarProps) {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-[#1a1a1a] border-b-[3px] border-[#ff9933]">
      {/* Single header bar: Logo · Nav · Auth */}
      <div className="mx-auto flex h-14 max-w-7xl items-center justify-between gap-6 px-4 lg:px-6">

        {/* Logo */}
        <Link href="/" className="flex shrink-0 items-center gap-2">
          <LogoIcon className="size-7 text-[#ff9933]" />
          <span className="text-xl font-bold tracking-tight text-[#fdf6e8] font-display">
            Daivik <span className="text-[#ff9933]">Vani</span>
          </span>
        </Link>
          ...
          {mainNavLinkKeys.map((link, i) => {
            ...
            return (
              <React.Fragment key={link.key}>
                {i > 0 && <span className="select-none text-[#ff9933]/20">|</span>}
                <Link
                  href={link.href}
                  className={`px-3 py-1 text-[13px] font-semibold uppercase tracking-wide transition-colors ${
                    isActive ? "text-[#ff9933]" : "text-[#fdf6e8]/70 hover:text-[#ff9933]"
                  }`}
                >
                  {t(link.key)}
                </Link>
        ...
        <div className="flex items-center gap-3">
          <div className="hidden lg:flex">
            {session?.user ? (
              <DropdownMenu>
                <DropdownMenuTrigger className="flex cursor-pointer items-center gap-2 rounded-md border border-[#ff9933]/30 px-3 py-1.5 text-sm font-medium text-[#fdf6e8]/80 transition-colors hover:border-[#ff9933] hover:bg-[#ff9933]/5">
                  <Avatar className="h-6 w-6">
                    {session.user.image && (
                      <AvatarImage src={session.user.image} alt={session.user.name ?? "User avatar"} />
                    )}
                    <AvatarFallback className="bg-[#ff9933] text-[10px] font-bold text-[#1a1a1a]">
                      {getInitials(session.user.name)}
                    </AvatarFallback>
                  </Avatar>
                  <span className="max-w-[100px] truncate">{session.user.name}</span>
                  <ChevronDown className="size-3 text-[#fdf6e8]/40" />
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" sideOffset={8} className="w-56 bg-[#1a1a1a] border-[#333]">
                  <DropdownMenuLabel>
                    <div className="flex flex-col gap-0.5">
                      <span className="text-sm font-medium text-[#fdf6e8]">{session.user.name}</span>
                      <span className="text-xs text-[#fdf6e8]/50">{session.user.email}</span>
                    </div>
                  </DropdownMenuLabel>
                  <DropdownMenuSeparator className="bg-[#333]" />
                  <DropdownMenuItem className="cursor-pointer text-[#fdf6e8]/80 focus:bg-[#ff9933]/10 focus:text-[#fdf6e8]" render={<Link href="/dashboard" />}>
                    <LayoutDashboard className="size-4 mr-2" /> Dashboard
                  </DropdownMenuItem>
                  <DropdownMenuItem className="cursor-pointer text-[#fdf6e8]/80 focus:bg-[#ff9933]/10 focus:text-[#fdf6e8]" render={<Link href="/profile" />}>
                    <User className="size-4 mr-2" /> Profile
                  </DropdownMenuItem>
                  {session.user.role === "ADMIN" && (
                    <DropdownMenuItem className="cursor-pointer text-[#fdf6e8]/80 focus:bg-[#ff9933]/10 focus:text-[#fdf6e8]" render={<Link href="/admin" />}>
              ...
              <Link
                href="/login"
                className="rounded-md border border-[#ff9933] bg-[#ff9933]/10 px-4 py-1.5 text-xs font-bold uppercase tracking-wide text-[#ff9933] transition-colors hover:bg-[#ff9933] hover:text-[#1a1a1a]"
              >
                {t("nav.signIn")}
              </Link>
            )}
          ...
          <button
            ...
            className="inline-flex items-center justify-center rounded-md border border-[#ff9933]/20 p-2 text-[#fdf6e8]/70 transition-colors hover:bg-[#ff9933]/10 hover:text-[#ff9933] lg:hidden"
      ...
      {/* Mobile Menu Overlay */}
      {mobileOpen && (
        <div className="fixed inset-0 top-[60px] z-40 overflow-y-auto bg-[#1a1a1a] lg:hidden">
